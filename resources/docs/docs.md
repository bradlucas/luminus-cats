...
The `home-routes` handler in the `luminus-cats.routes.home` namespace
...
The `home-page` function will in turn call the `luminus-cats.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `luminus-cats.handler` namespace:
...
Request middleware functions are located under the `luminus-cats.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `luminus-cats.dev-middleware` namespace found in
